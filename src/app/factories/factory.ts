...
export type Factory = FactoryProps & {
  ...
  setBuilding(
    buildingId: string,
    update: (
      state: Factory["buildings"][number],
    ) => Factory["buildings"][number],
...
export function producedBy(
  ...
  return (
    building.data.count *
    (building.data.recipe.produces.find((ir) => ir.item === itemId)?.rate ?? 0)
  );
}
...
export function requiredBy(
  ...
  return (
    building.data.count *
    (building.data.recipe.requires.find((ir) => ir.item === itemId)?.rate ?? 0)
  );
}
...
export function availableCount(factory: Factory, handleId: string) {
  ...
  const connectionsWithItem = factory.connections.flatMap((c) => {
    ...
    const sourceBuilding = factory.buildings.find(
      (b) => b.id === source.buildingId,
    );
    const targetBuilding = factory.buildings.find(
      (b) => b.id === target.buildingId,
    );
  ...
  for (const building of factory.buildings) {
    ...
    for (const connection of connectionsWithItem) {
      ...
      const required = requiredBy(
        factory,
        connection.targetBuilding.id,
        itemId,
      );
    ...
    if (lastSeen)
      consumed.set(lastSeen, (consumed.get(lastSeen) ?? 0) + available);
  }
...
export function unconnected(factory: Factory) {
  const inputHandles = factory.buildings.flatMap((b) =>
    b.data.recipe.requires.map((i) => handleId(b.id, true, i.item)),
  );
  const outputHandles = factory.buildings.flatMap((b) =>
    b.data.recipe.produces.map((i) => handleId(b.id, false, i.item)),
  );
  const unconnected = [...inputHandles, ...outputHandles].filter(
    (handle) =>
      !factory.connections.find(
...
function itemRates(input: string, secondsToCreate: number): ItemRate[] {
  ...
  for (const [match] of input.matchAll(itemRateRegex)) {
    ...
    results.push({
      item: normalize(item[1]),
      rate: (parseFloat(amount[1]) * 60) / secondsToCreate,
    });
  }
...
function bestMatch(
  ...
  const leftItems = (searchInput ? left.requires : left.produces).map(
    (ir) => ir.item,
  );
  const rightItems = (searchInput ? right.requires : right.produces).map(
    (ir) => ir.item,
  );
  if (!leftItems.includes(requiredItem)) return right;
  if (!rightItems.includes(requiredItem)) return left;
  const leftMatches = searchRates.filter((ir) =>
    leftItems.includes(ir.item),
  ).length;
  const rightMatches = searchRates.filter((ir) =>
    rightItems.includes(ir.item),
  ).length;
  return leftMatches >= rightMatches ? left : right;
}
const getBestRecipe = (
  rates: ItemRate[],
  ratesAreOutput: boolean,
  requiredItem: string,
) =>
  recipes.reduce(
    (prev, curr) => bestMatch(rates, ratesAreOutput, prev, curr, requiredItem),
    defaultRecipe,
  );
      ...
      (set, get) => ({
        ...
        setBuilding(buildingId, update) {
          set((state) => {
            ...
            if (!building)
              throw new Error("Tried to set non-existent building");
        ...
        add(params) {
          const recipe = params?.recipe
            ? params.recipe
            : get().buildings.length === 0
              ? defaultRecipe
              : get().buildings[0].data.recipe;

          const position = params?.position ?? {
            x:
              get().buildings.length === 0
                ? 0
                : this.buildings[0].position.x + 50,
            y:
              get().buildings.length === 0
                ? 0
                : this.buildings[0].position.y + 50,
          };
        ...
        setRecipe(buildingId, recipeId) {
          ...
          const toRemove = factory.connections.filter((c) => {
            if (c.source !== buildingId && c.target !== buildingId)
              return false;

            if (!c.sourceHandle || !c.targetHandle)
              throw new Error("Invalid connection to remove.");

            const sourceHandle = reverseHandleId(c.sourceHandle);
            const targetHandle = reverseHandleId(c.targetHandle);

            if (
              sourceHandle.buildingId === buildingId &&
              recipe.produces.some((i) => i.item === sourceHandle.itemId)
            ) {
              return false;
            }
            if (
              targetHandle.buildingId === buildingId &&
              recipe.requires.some((i) => i.item === targetHandle.itemId)
            ) {
              return false;
            }
          ...
          return factory.setBuilding(buildingId, (building) => ({
            ...building,
            data: { ...building.data, recipe: recipe },
          }));
        },
        ...
        onConnectEnd(releasePosition, connectionState, reactFlow) {
          if (!connectionState.toNode) {
            ...
            if (!sourceNode || !isBuilding(sourceNode?.data) || !sourceHandle)
              return;
            const sourceIsOutput = sourceHandle.type === "source";
            const requiredItem = reverseHandleId(sourceHandle.id!).itemId;
            const targetRecipe = getBestRecipe(
              sourceIsOutput
                ? sourceNode.data.recipe.produces
                : sourceNode.data.recipe.requires,
              sourceIsOutput,
              requiredItem,
            );
            ...
            setTimeout(() => {
              const target = get().buildings.find(
                (b) => b.id === targetNode?.id,
              );
              const source = get().buildings.find(
              ...
              if (target && source) {
                const realSourceHandle = sourceIsOutput
                  ? sourceHandle.id
                  : targetHandleId;
                const realTargetHandle = sourceIsOutput
                  ? targetHandleId
                  : sourceHandle.id;
